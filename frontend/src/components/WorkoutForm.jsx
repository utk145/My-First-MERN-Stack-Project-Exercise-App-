...
const WorkoutForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!response.ok) {
            setError(workoutObjs.error)
            setEmptyFields(workoutObjs.emptyFields)
        }
        if (response.ok) {
            setTitle('')
            setLoad('')
            ...
            setError(null)
            setEmptyFields([])
            console.log('New workout is added', workoutObjs)
            dispatch({ type: 'CREATE_WORKOUT', payload: workoutObjs })
        }
    };
